        ...
        <div class="w-full max-w-4xl">
          <div class="flex flex-col sm:flex-row justify-between sm:items-center mb-4 gap-4">
            ...
            <div class="flex-shrink-0 flex gap-2">
                <a href={`/event/${params.id}?view=add`} class={`px-4 py-2 rounded-md text-sm font-semibold ${view === 'add' ? 'bg-sky-600 text-white shadow' : 'bg-white dark:bg-slate-700'}`}>{t.add_participant_view}</a>
                <a href={`/event/${params.id}?view=list`} class={`px-4 py-2 rounded-md text-sm font-semibold ${view === 'list' ? 'bg-sky-600 text-white shadow' : 'bg-white dark:bg-slate-700'}`}>{t.list_participants_view}</a>
            </div>
          </div>
